...
public static class MUtils
{
	...
	// }
	/// <summary>
	/// 带符号线性增长（原linearAttack）
	/// 功能：根据数值正负方向增长，不超过最大绝对值限制
	/// </summary>
	/// <param name="val">当前数值</param>
	/// <param name="accDelta">增长幅度（增量）</param>
	/// <param name="max">最大绝对值限制</param>
	/// <returns>增长后的数值</returns>
	public static float LinearAttack(float val, float accDelta, float max)
	{
		if (val > 0)
		{
			// 正值：向上增长，不超过max
			return Mathf.Min(max, val + accDelta);
		}
		else
		{
			// 负值/零：向下增长（绝对值增大），不低于 -max
			return Mathf.Max(-max, val - accDelta);
		}
	}

	/// <summary>
	/// 带符号线性衰减（原linearDecay）
	/// 功能：根据数值正负方向衰减，最终趋近于0
	/// </summary>
	/// <param name="val">当前数值</param>
	/// <param name="accDelta">衰减幅度（减量）</param>
	/// <returns>衰减后的数值</returns>
	public static float LinearDecay(float val, float accDelta)
	{
		if (val > 0)
		{
			// 正值：向下衰减，不低于0
			return Mathf.Max(0, val - accDelta);
		}
		else
		{
			// 负值/零：向上衰减（绝对值减小），不高于0
			return Mathf.Min(0, val + accDelta);
		}
	}

	/// <summary>
	/// 数值叠加处理（原pluse）
	/// 功能：根据当前值与叠加值的符号关系，返回不同叠加结果，支持强制替换
	/// </summary>
	/// <param name="val">当前数值</param>
	/// <param name="p">叠加值</param>
	/// <param name="force">是否强制用叠加值替换（默认false）</param>
	/// <returns>叠加后的数值</returns>
	public static float Pluse(float val, float p, bool force = false)
	{
		if (val * p <= 0)
		{
			// 情况1：当前值与叠加值异号或至少一个为0
			return force ? p : val + p;
		}
		else if (val > 0 && p > 0)
		{
			// 情况2：两者均为正值：取较小值（若当前值小于叠加值，用叠加值；否则保留当前值）
			return val < p ? p : val;
		}
		else
		{
			// 情况3：两者均为负值：取较大值（若当前值大于叠加值，用叠加值；否则保留当前值）
			return val > p ? p : val;
		}
	}
	...
	/// <summary>
	/// 数值趋向目标值（原moveTo）
	/// 功能：按指定步长向目标值移动，若步长超过剩余距离，直接返回目标值
	/// </summary>
	/// <param name="cur">当前数值</param>
	/// <param name="delta">每次移动的步长（可正可负，代表移动方向）</param>
	/// <param name="target">目标数值</param>
	/// <returns>移动后的数值（若超过目标则返回目标值）</returns>
	public static float MoveTo(float cur, float delta, float target)
	{
		float next = cur + delta;
		// 判断：移动后的数值是否跨越了目标值（乘积小于0表示方向相反，即跨越）
